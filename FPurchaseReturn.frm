...
Begin VB.Form FPurchaseReturn 
   ...
   Begin VB.CommandButton CAddItem 
      Height          =   500
      Left            =   300
      Picture         =   "FPurchaseReturn.frx":1FEC42
      Style           =   1  'Graphical
      TabIndex        =   9
      Top             =   6645
      Width           =   1365
   ...
   Begin VB.CommandButton CRemoveItem 
      Height          =   500
      Left            =   1740
      Picture         =   "FPurchaseReturn.frx":2010A4
      Style           =   1  'Graphical
      TabIndex        =   15
      Top             =   6645
      Width           =   1365
   ...
   Begin VB.CommandButton CClear 
      Height          =   500
      Left            =   3180
      Picture         =   "FPurchaseReturn.frx":203506
      Style           =   1  'Graphical
      TabIndex        =   14
      Top             =   6645
      Width           =   1365
   ...
   Begin VB.CommandButton CNew 
      Height          =   500
      Left            =   315
      Picture         =   "FPurchaseReturn.frx":205968
      Style           =   1  'Graphical
      TabIndex        =   13
      Top             =   7320
      Width           =   1365
   ...
   Begin VB.CommandButton CSave 
      Height          =   500
      Left            =   9195
      Picture         =   "FPurchaseReturn.frx":207DCA
      Style           =   1  'Graphical
      TabIndex        =   12
      Top             =   7335
      Width           =   1365
   ...
   Begin VB.CommandButton CClose 
      ...
      Height          =   500
      Left            =   10635
      Picture         =   "FPurchaseReturn.frx":20A22C
      Style           =   1  'Graphical
      TabIndex        =   11
      Top             =   7335
      Width           =   1365
   ...
   Begin VB.CommandButton CDelete 
      Height          =   500
      Left            =   4740
      Picture         =   "FPurchaseReturn.frx":20C68E
      Style           =   1  'Graphical
      TabIndex        =   10
      Top             =   75
      Width           =   1365
   ...
   Begin MSForms.Label Label2 
      ...
      BackColor       =   15724527
      Size            =   "21775;873"
      Picture         =   "FPurchaseReturn.frx":20EAF0
      FontHeight      =   165
      FontCharSet     =   0
      FontPitchAndFamily=   2
   End
...
Dim rs As Recordset, sTransactionNo As String
    ...
    Set rs = db.OpenRecordset("Select Max(Val( Transaction.TransactionNo)) As TNo From Transaction Where ( Transaction.TransactionType = 'PR' )")
...
Dim rs As Recordset, r As Long
    ...
    Set rs = db.OpenRecordset("Select Sum(Transaction.Quantity) As Quantity From Transaction Where (Transaction.TransactionNo='" & Trim(TTransactionNo.Text) & "') And (Transaction.TransactionType='PR') And (Transaction.ItemCode = '" & sItemCode(CoItem.ListIndex + 1) & "')")
    If rs.RecordCount > 0 Then
...
Dim rs As Recordset, lYN As Long, bFound As Boolean
    ...
    If (MsgBox("Do you want to Delete the Bill ?", vbDefaultButton2 Or vbYesNo) = vbYes) Then
        Set rs = db.OpenRecordset("Select Transaction.* From Transaction Where (Transaction.TransactionNo = '" & Trim(TTransactionNo.Text) & "' ) And (Transaction.TransactionType = 'PR' )")
        While rs.EOF = False
            bFound = True
            rs.Delete
            rs.MoveNext
...
Dim rs As Recordset, r As Long
    ...
    Set rs = db.OpenRecordset("Select ItemMaster.ItemName,ItemMaster.BillingName,ItemMaster.Code,Transaction.Tax as ItemTax,Units.UnitName,Transaction.* From ItemMaster,Transaction,Units Where (Transaction.TransactionNo = '" & Trim(TTransactionNo.Text) & "' ) And (Transaction.TransactionType = 'PR' ) And (ItemMaster.Code = Transaction.ItemCode ) And (Units.Code = ItemMaster.UnitCode ) Order By Transaction.SerialNo")
    MGrid.Rows = 0
    If rs.RecordCount > 0 Then
