...
Begin VB.Form FOutstandingReceivable 
   Caption         =   "Outstanding Receivable"
   ClientHeight    =   6705
   ClientLeft      =   60
   ClientTop       =   450
   ClientWidth     =   10905
   LinkTopic       =   "Form1"
   LockControls    =   -1  'True
   Picture         =   "FOutstandingReceivable.frx":0000
   ScaleHeight     =   6705
   ScaleWidth      =   10905
   ...
   Begin VB.CommandButton CShow 
      ...
      Height          =   500
      Left            =   240
      Picture         =   "FOutstandingReceivable.frx":1FEC42
      Style           =   1  'Graphical
      TabIndex        =   2
      Top             =   6030
      Width           =   1365
   ...
   Begin VB.CommandButton CToExcel 
      ...
      Height          =   500
      Left            =   7635
      Picture         =   "FOutstandingReceivable.frx":2010A4
      Style           =   1  'Graphical
      TabIndex        =   1
      Top             =   6030
      Width           =   1365
   ...
   Begin VB.CommandButton CClose 
      ...
      Height          =   500
      Left            =   9315
      Picture         =   "FOutstandingReceivable.frx":203506
      Style           =   1  'Graphical
      TabIndex        =   0
      Top             =   6015
      Width           =   1365
   ...
   Begin MSForms.Label Label5 
      ...
      BackColor       =   15724527
      Size            =   "18891;873"
      Picture         =   "FOutstandingReceivable.frx":205968
      FontHeight      =   165
      FontCharSet     =   0
      FontPitchAndFamily=   2
   End
End
Attribute VB_Name = "FOutstandingReceivable"
...
Dim rs As Recordset
    ...
    ElseIf (CoType.ListIndex > 0) Then
        ...
        Set rs = db.OpenRecordset("Select AccountRegister.Code,AccountRegister.AccountName From AccountRegister Where (AccountRegister.GroupCode='" & sCustomerAccountParentID & "' )")
    Else
        Exit Sub
    End If
...
Dim rs As Recordset, dOpeningBalance As Double, dBalance As Double, sCode As String
    ...
    MGrid.Rows = 0
    If (CoType.ListIndex = 0) Then
            Set rs = db.OpenRecordset("Select Sum(AR.Debit-AR.Credit) As OpeningBalance From AccountTransaction As AR Where (AR.GCode = '" & sCustomerAccountParentID & "') And (AR.EditedDate < cDate('" & DTPFrom.Value & "')) ")
            If rs.RecordCount > 0 Then
                dOpeningBalance = Val("" & rs!OpeningBalance)
            End If
            
            Set rs = db.OpenRecordset("Select AccountRegister.AccountName,AccountTransaction.* From AccountRegister,AccountTransaction Where (AccountRegister.Code = AccountTransaction.AccountCode )  And (AccountTransaction.GCode = '" & sCustomerAccountParentID & "' ) And (AccountTransaction.EditedDate Between cDate('" & DTPFrom.Value & "') And cDate('" & DTPTo.Value & "') ) Order By AccountTransaction.EditedDate,Val(AccountTransaction.BillNo)")
...
Dim lRowCount As Long, lColCount As Long
    ...
    oExcelSheet.Cells(1, 1).Value = "Outstanding Receivables From " & Format(DTPFrom.Value, "dd-MM-yyyy") & " To " & Format(DTPTo.Value, "dd-MM-yyyy")

    oExcelSheet.Range("A1:F" & lRowCount + 4).Select
    oExcel.Application.Selection.AutoFormat
On Error Resume Next

    Kill App.Path & "\Reports\Outstanding Receivable " & Format(Date, "dd-MMM-yyyy") & ".xlsx"

    oExcel.SaveAs App.Path & "\Reports\Outstanding Receivable " & Format(Date, "dd-MMM-yyyy") & ".xlsx"
    Set oExcel = Nothing
    ...
    xlTmp.Visible = True
    xlTmp.Workbooks.Open App.Path & "\Reports\Outstanding Receivable " & Format(Date, "dd-MMM-yyyy") & ".xlsx"
...
Private Sub Form_Load()
    ...
    MGridInitialise
    CoType.AddItem "All"
    CoType.AddItem "Select Customer"
End Sub
