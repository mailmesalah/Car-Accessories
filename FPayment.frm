...
Begin VB.Form FPayment 
   BorderStyle     =   1  'Fixed Single
   Caption         =   "Accounts - Payment"
   ClientHeight    =   4635
   ClientLeft      =   45
   ClientTop       =   435
   ClientWidth     =   6360
...
Dim rs As Recordset
    ...
    Set rs = db.OpenRecordset("Select AccountTransaction.* From AccountTransaction Where (AccountTransaction.BillNo = '" & Trim(TVoucherNo.Text) & "' ) And (AccountTransaction.Type = 'P' ) And (AccountTransaction.FinancialCode='" & getFinancialCode(DTPDate.Value) & "')")
...
Dim rs As Recordset, sTransactionNo As String
    ...
    Set rs = db.OpenRecordset("Select Max(Val(AccountTransaction.BillNo)) As TNo From AccountTransaction Where (AccountTransaction.Type = 'P') And (AccountTransaction.FinancialCode='" & getFinancialCode(DTPDate.Value) & "')")
