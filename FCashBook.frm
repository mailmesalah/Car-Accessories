...
Begin VB.Form FCashBook 
   ...
   ClientWidth     =   14220
   ControlBox      =   0   'False
   Icon            =   "FCashBook.frx":0000
   KeyPreview      =   -1  'True
   LinkTopic       =   "Form1"
   LockControls    =   -1  'True
   Picture         =   "FCashBook.frx":000C
   ScaleHeight     =   6570
   ScaleWidth      =   14220
...
Dim dDebit As Double, dCredit As Double
    ...
    Set rs = db.OpenRecordset("Select (Sum(AccountTransaction.Debit)-Sum(AccountTransaction.Credit)) As OpeningBalance From AccountTransaction Where (AccountTransaction.AccountCode<>'" & sCashAccount & "') And (AccountTransaction.Mode='Cash') And (AccountTransaction.EditedDate < cDate('" & DTPFrom.Value & "'))")
...
Dim lRowCount As Long, lColCount As Long
  ...
  ' Assign the data to Excel...
    ...
    oExcelSheet.Cells(1, 1).Value = "Cash Book From " & Format(DTPFrom.Value, "dd-MM-yyyy") & " To " & Format(DTPTo.Value, "dd-MM-yyyy")
...
On Error Resume Next
    ...
    Kill App.Path & "\Reports\Cash Book " & Format(Date, "dd-MMM-yyyy") & ".xlsx"

  ' Save the file as a native XLS file...
    oExcel.SaveAs App.Path & "\Reports\Cash Book " & Format(Date, "dd-MMM-yyyy") & ".xlsx"
  ...
  ' Close the OLE object and remove it...
    ...
    xlTmp.Visible = True
    xlTmp.Workbooks.Open App.Path & "\Reports\Cash Book " & Format(Date, "dd-MMM-yyyy") & ".xlsx"
...
Dim r As Long, daDate As Date
    If MGrid.Rows > 0 Then
        ...
        ElseIf MGrid.TextMatrix(MGrid.Row, gType) = "O" Then
            FOpeningBalance.Show
            FOpeningBalance.TVoucherNo = MGrid.TextMatrix(MGrid.Row, gBillNo)
            FOpeningBalance.DTPDate.Value = daDate
            FOpeningBalance.getTransactionDetails MGrid.TextMatrix(MGrid.Row, gBillNo)
        ...
        ElseIf MGrid.TextMatrix(MGrid.Row, gType) = "SB" Then
            FRetailSales.Show
            FRetailSales.TTransactionNo = MGrid.TextMatrix(MGrid.Row, gBillNo)
            FRetailSales.DTPDate.Value = daDate
            FRetailSales.getTransactionDetails
        ElseIf MGrid.TextMatrix(MGrid.Row, gType) = "S8B" Then
            FSaleReturn.Show
            FSaleReturn.TTransactionNo = MGrid.TextMatrix(MGrid.Row, gBillNo)
            FSaleReturn.DTPDate.Value = daDate
            FSaleReturn.getTransactionDetails
        End If
    End If
End Sub
