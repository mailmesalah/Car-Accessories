...
Begin VB.Form FOpeningStock 
   ...
   KeyPreview      =   -1  'True
   LinkTopic       =   "Form1"
   Picture         =   "FOpeningStock.frx":0000
   ScaleHeight     =   8070
   ScaleWidth      =   12540
   ...
   Begin VB.CommandButton CDelete 
      Height          =   500
      Left            =   4800
      Picture         =   "FOpeningStock.frx":1FEC42
      Style           =   1  'Graphical
      TabIndex        =   17
      Top             =   120
      Width           =   1365
   ...
   Begin VB.CommandButton CClose 
      ...
      Height          =   500
      Left            =   10695
      Picture         =   "FOpeningStock.frx":2010A4
      Style           =   1  'Graphical
      TabIndex        =   16
      Top             =   7425
      Width           =   1365
   ...
   Begin VB.CommandButton CSave 
      Height          =   500
      Left            =   9255
      Picture         =   "FOpeningStock.frx":203506
      Style           =   1  'Graphical
      TabIndex        =   15
      Top             =   7425
      Width           =   1365
   ...
   Begin VB.CommandButton CNew 
      Height          =   500
      Left            =   375
      Picture         =   "FOpeningStock.frx":205968
      Style           =   1  'Graphical
      TabIndex        =   14
      Top             =   7410
      Width           =   1365
   ...
   Begin VB.CommandButton CClear 
      Height          =   500
      Left            =   3240
      Picture         =   "FOpeningStock.frx":207DCA
      Style           =   1  'Graphical
      TabIndex        =   13
      Top             =   6735
      Width           =   1365
   ...
   Begin VB.CommandButton CRemoveItem 
      Height          =   500
      Left            =   1800
      Picture         =   "FOpeningStock.frx":20A22C
      Style           =   1  'Graphical
      TabIndex        =   12
      Top             =   6735
      Width           =   1365
   ...
   Begin VB.CommandButton CAddItem 
      Height          =   500
      Left            =   360
      Picture         =   "FOpeningStock.frx":20C68E
      Style           =   1  'Graphical
      TabIndex        =   8
      Top             =   6735
      Width           =   1365
   ...
   Begin MSForms.Label Label2 
      ...
      BackColor       =   15724527
      Size            =   "21775;873"
      Picture         =   "FOpeningStock.frx":20EAF0
      FontHeight      =   165
      FontCharSet     =   0
      FontPitchAndFamily=   2
   End
   ...
   End
End
Attribute VB_Name = "FOpeningStock"
...
Dim rs As Recordset, sTransactionNo As String
    ...
    Set rs = db.OpenRecordset("Select Max(Val( Transaction.TransactionNo)) As TNo From Transaction Where ( Transaction.TransactionType = 'O' )")
...
Dim rs As Recordset, r As Long
    ...
    Set rs = db.OpenRecordset("Select Sum(Transaction.Quantity) As Quantity From Transaction Where (Transaction.TransactionNo='" & Trim(TTransactionNo.Text) & "') And (Transaction.TransactionType='O') And (Transaction.ItemCode = '" & sItemCode(CoItem.ListIndex + 1) & "')")
    If rs.RecordCount > 0 Then
...
Dim rs As Recordset, lYN As Long, bFound As Boolean
    ...
    If (MsgBox("Do you want to Delete the Bill ?", vbDefaultButton2 Or vbYesNo) = vbYes) Then
        Set rs = db.OpenRecordset("Select Transaction.* From Transaction Where (Transaction.TransactionNo = '" & Trim(TTransactionNo.Text) & "' ) And (Transaction.TransactionType = 'O' )")
        While rs.EOF = False
            bFound = True
            rs.Delete
            rs.MoveNext
...
Dim rs As Recordset
    Set rs = db.OpenRecordset("Select AccountRegister.* From AccountRegister Where (AccountRegister.BillNo = '" & Trim(TTransactionNo.Text) & "' ) And (AccountRegister.SpecialAccount In( 'Purchase','PBillVoucher' ))")
    While rs.EOF = False
        rs.Delete
        rs.MoveNext
    Wend
...
Dim r As Long, lYN As Long, sStatus As String
    ...
    While r < MGrid.Rows
        ...
        rs!TransactionType = "O"
        rs!TransactionDate = DateValue("01/01/1900")
...
Dim rs As Recordset, r As Long
    ...
    Set rs = db.OpenRecordset("Select ItemMaster.ItemName,ItemMaster.BillingName,ItemMaster.Code,Transaction.Tax as ItemTax,Units.UnitName,Transaction.* From ItemMaster,Transaction,Units Where (Transaction.TransactionNo = '" & Trim(TTransactionNo.Text) & "' ) And (Transaction.TransactionType = 'O' ) And (ItemMaster.Code = Transaction.ItemCode ) And (Units.Code = ItemMaster.UnitCode ) Order By Transaction.SerialNo")
    MGrid.Rows = 0
    If rs.RecordCount > 0 Then
...
Dim rs As Recordset, sTransactionNo As String
    ...
    Set rs = db.OpenRecordset("Select AccountRegister.* From AccountRegister Where (AccountRegister.SpecialAccount = 'Purchase' )And (AccountRegister.Type In ('P','R') )And (AccountRegister.BillNo = '" & TTransactionNo.Text & "') Order By AccountRegister.Type ")
    If (rs.RecordCount > 0) Then
    ...
    rs!TransactionTime = Format(Time, "HH:MM AMPM")
    rs!AccountCode = IIf(CoSupplier.ListIndex = -1, Trim(sGeneralSupplierAccountID), sAccountCode(CoSupplier.ListIndex + 1))
    rs!Narration = "- Bill Amount"
    rs!CashOrCredit = "Credit"
    rs!Expense = 0
    rs!Income = Val(LGrandAmount.Caption)
    rs!BillNo = "" & TTransactionNo.Text
    rs!SpecialAccount = "Purchase"
    rs.Update
    ...
    rs!TransactionTime = Format(Time, "HH:MM AMPM")
    rs!AccountCode = IIf(CoSupplier.ListIndex = -1, Trim(sGeneralSupplierAccountID), sAccountCode(CoSupplier.ListIndex + 1))
    rs!Narration = "- Payment"
    rs!CashOrCredit = "Cash"
    rs!Expense = Val(TPayment.Text)
    rs!Income = 0
    rs!BillNo = "" & TTransactionNo.Text
    rs!SpecialAccount = "Purchase"
    rs.Update
