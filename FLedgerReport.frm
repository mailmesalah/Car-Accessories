...
Dim lRowCount As Long, lColCount As Long
    ...
    oExcelSheet.Cells(1, 1).Value = "Company Name"
    oExcelSheet.Cells(2, 1).Value = "Address"
    oExcelSheet.Cells(3, 1).Value = "Address"
...
On Error Resume Next
    ...
    Kill App.Path & "\Reports\Ledger Report" & Format(Date, "dd-MMM-yyyy") & ".xlsx"

    oExcel.SaveAs App.Path & "\Reports\Ledger Report" & Format(Date, "dd-MMM-yyyy") & ".xlsx"
    Set oExcel = Nothing
    ...
    xlTmp.Visible = True
    xlTmp.Workbooks.Open App.Path & "\Reports\Ledger Report" & Format(Date, "dd-MMM-yyyy") & ".xlsx"
...
Dim r As Long, daDate As Date
    If MGrid.Rows > 0 Then
        ...
        ElseIf MGrid.TextMatrix(MGrid.Row, gType) = "O" Then
            ...
            FOpeningBalance.TVoucherNo = MGrid.TextMatrix(MGrid.Row, gVoucherNo)
            FOpeningBalance.DTPDate.Value = daDate
            FOpeningBalance.getTransactionDetails MGrid.TextMatrix(MGrid.Row, gVoucherNo)
        ...
        ElseIf MGrid.TextMatrix(MGrid.Row, gType) = "SB" Then
            FRetailSales.Show
            FRetailSales.TTransactionNo = MGrid.TextMatrix(MGrid.Row, gVoucherNo)
            FRetailSales.DTPDate.Value = daDate
            FRetailSales.getTransactionDetails
        ElseIf MGrid.TextMatrix(MGrid.Row, gType) = "S8B" Then
            FSaleReturn.Show
            FSaleReturn.TTransactionNo = MGrid.TextMatrix(MGrid.Row, gVoucherNo)
